// Displays courier account information and allows for updating of account information
...
const CourierAccount = (navigation) => {
  ...
  // Function to update the courier account information
  const updateAccount = () => {
    fetch(`${URI}/api/account/${user.courier_id}`, {
      method: "POST",
      headers: {
  ...
  // Fetch the courier account information
  useEffect(() => {
    fetch(`${URI}/api/account/${user.user_id}?type=courier`)
      .then((response) => response.json())
      .then((data) => {
        setPrimaryEmail(data.primary_email);
        setAccountEmail(data.account_email);
    ...
    >
      <View style={styles.container}>
        ...
        <View style={styles.page}>
          ...
          <Text style={styles.labelText}>Courier Email:</Text>
          <TextInput
            style={styles.textInputBox}
            value={accountEmail}
            onChangeText={(text) => {
          ...
          <Text style={styles.labelText}>Courier Phone:</Text>
          <TextInput
            style={styles.textInputBox}
            value={accountPhone}
            onChangeText={(text) => {
          ...
          <TouchableOpacity
            style={[
              styles.button,
              hasUnsavedChanges ? styles.buttonActive : styles.buttonInactive,
            ]}

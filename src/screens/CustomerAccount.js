// Display and update customer account information
...
const CustomerAccount = (navigation) => {
  ...
  // Function to update the customer account information
  const updateAccount = () => {
  ...
  // Fetch the customer account information
  useEffect(() => {
    fetch(`${URI}/api/account/${user.user_id}?type=customer`)
      .then((response) => response.json())
      .then((data) => {
        setPrimaryEmail(data.primary_email);
        setAccountEmail(data.account_email);
